...
func (pdb *PostgresDb) Checkin(id, copies string) (*models.Book, error) {
	...
	if err := pdb.DB.Model(book).Where("id", id).Update("quantity", newQuantity).Error; err != nil {
		return nil, err
	}
	return book, nil
}
...
func (pdb *PostgresDb) Deletebook(id string) error {
	if err := pdb.DB.Delete(book).Where("id= ?", id).Error; err != nil {
		return err
	}
	return nil
}
